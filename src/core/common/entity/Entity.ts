...
export class Entity<TIdentifier extends string|number> {
  ...
  public getId(): TIdentifier {
    if (typeof this.id === 'undefined') {
      throw Exception.new({code: Code.ENTRY_VALIDATION_ERROR, overrideMessage: `${this.constructor.name}: ID is empty.`});
    }
  ...
  public async validate(): Promise<void> {
    ...
    if (details) {
      throw Exception.new({code: Code.ENTRY_VALIDATION_ERROR, data: details});
    }
  }
